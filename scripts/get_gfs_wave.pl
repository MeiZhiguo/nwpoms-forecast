...
# 0.5x0.5 degree GFS
#$URL='https://nomads.ncep.noaa.gov:443/pub/data/nccf/com/gfs/prod/gfs.$YYYY$MM$DD/$HH/atmos/gfs.t${HH}z.pgrb2full.0p50.f${FHR3}';
# 0.25 x 0.25 degree GFS
 $URL='https://nomads.ncep.noaa.gov:443/pub/data/nccf/com/gfs/prod/gfs.$YYYY$MM$DD/$HH/wave/gridded/gfswave.t${HH}z.global.0p16.f${FHR3}.grib2';
...
while ($fhr <= $hr1) {
   ...
   if ($windows eq 'yes') {
      ...
      $err = $err >> 8;
       while( $err != 0 ){
          printf "Connected error! Try download again after 5 second";
          sleep(5);
          $err = system("$curl $insecure -f -s $url$inv -o $OUTDIR/$file.tmp");
       }
#      if ($err) {
#         print "$curl $insecure -f -s $url$inv -o $OUTDIR/$file.tmp";
...
#      if ($err != 0) {
#         print "$curl $insecure -f -v -s -r \"$range\" $url$grb -o $OUTDIR/$file.tmp";
